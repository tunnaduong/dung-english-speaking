...
class AdminController extends Controller
{
  ...
  public function addEmployee()
  {
    if (request()->isMethod('post')) {
      $rules = [
        ...
        'gender' => 'required',
        'personal_id' => 'required|numeric|min:12|max:12',
        'DoB' => 'required',
  ...
  public function editEmployee($id)
  {
    ...
    if ($currentUser && $currentUser['user_id'] == $id) {
      flashError(['error' => 'You cannot edit your own account']);
      return redirect('/employees');
    }

    // Get employee info to check if it's a demo account
    ...
    if ($employee) {
      ...
      if (in_array($employee['email'], $demoEmails)) {
        flashError(['error' => 'You cannot edit demo accounts']);
        return redirect('/employees');
      }
    }
  ...
  public function deleteEmployee($id)
  {
    ...
    $currentUser = session('user');
    if ($currentUser && $currentUser['user_id'] == $id) {
      flashError(['error' => 'You cannot delete your own account']);
      return redirect('/employees');
    }
    ...
    if ($employee) {
      ...
      if (in_array($employee['email'], $demoEmails)) {
        flashError(['error' => 'You cannot delete demo accounts']);
        return redirect('/employees');
      }
    }
