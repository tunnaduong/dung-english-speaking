...
class Classroom extends Model
{
    ...
    {
        $sql = "SELECT c.id AS class_id, c.class_name, cu.date, s.day_of_week1, s.day_of_week2, s.shift1, s.shift2 FROM class c JOIN course co ON co.id = c.id_course JOIN curriculum cu ON cu.course_id = co.id JOIN schedules sc ON sc.id = cu.exercise_id LEFT JOIN schedules s ON s.id = sc.id WHERE cu.date IS NOT NULL";
